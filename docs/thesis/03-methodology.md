...
**Gate 05: Behavioral Physics Check** (`05-behavioral-physics-check.ts`). Verifies that the runtime values of behavioral constants ($\lambda = 2.0$, $\beta_c = 5$, $\beta_f = 15$, $\beta_s = 20$, $IS_0 = 50$, $\bar{\sigma} = 50000$, $\underline{\delta} = 7$, etc.) match the values specified in the formal definitions. This gate detects "constant drift" --- accidental modification of calibrated parameters during refactoring. In live deployment mode (enabled by the `CI_GATE05_API_URL` environment variable), the gate queries the running API to verify that deployed constants match specification.
