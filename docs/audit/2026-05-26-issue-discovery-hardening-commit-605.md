...
Falls back to `'sk_test_REDACTED_key'`; a config where the webhook secret is set but the API key is mock could yield inconsistent verification. Production should hard-fail on the mock key.
