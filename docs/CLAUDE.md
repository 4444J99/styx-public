...
**Styx** ("The Blockchain of Truth") — a peer-audited behavioral market that uses loss aversion (coefficient 2.0) to enforce habit follow-through via financial stakes. Users stake money into behavioral contracts; a decentralized "Fury" network audits compliance; hardware oracles and a double-entry ledger enforce integrity.

Turborepo monorepo in ORGAN-III (commercial products). Promotion status: **PUBLIC_PROCESS**.

## Build & Dev Commands

All commands run from the repo root. Package manager: **npm** (not yarn/pnpm). Node 24 LTS.
...
- **CI**: GitHub Actions (`ci.yml`) — Node 24 LTS, advisory dependency audit, turbo test + build + lint, Gates 04–07, beta readiness, Terraform validate, Playwright E2E (chromium + firefox), CodeQL
...
- **`ecosystem.yaml`**, **`network-map.yaml`** — Ecosystem inventory (`.config/pm2/ecosystem.yaml`) and external-mirror map (`docs/architecture/network-map.yaml`) used by `organvm ecosystem show` and `organvm network map`.
