 ...
 * Perceived pain of loss is ~2x pleasure of gain (λ = 2.0).
...
| Unified beta readiness contract + artifact (\`beta-readiness-summary.json\`) | Phase 1 release operations | Implemented | \`/scripts/smoke/beta-readiness.sh\`, \`/docs/planning/planning--beta-readiness-contract.md\`, \`/.github/workflows/beta-promotion.yml\` | \`npm run beta:readiness\` emits \`artifacts/beta-readiness-summary.json\`; the beta-promotion workflow uploads it in the \`beta_readiness\` job | Platform |
...
consumes:
  - type: theory
    ...
    description: "Behavioral economics models (loss aversion coefficient 2.0, prospect theory) and game-theoretic peer audit proofs"
